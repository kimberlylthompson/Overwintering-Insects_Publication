<<<<<<< HEAD
###########################################################################
###########      Figures for SCP Sensitivity in Papilio canadensis ########
...
# This script creates the map panels used in Figure 1 depicting the 
# worst-case scenario for days under the SCP of P. canadensis.

# This done for the SCP days based on the mean, median, min and max 
# predicted values of ground temperatures
# however only the mean is used in the analysis and in Figure 1.

# Jpgs produced:
# SCP_Pcanadensis_Worst_Case_Mean_External.jpg
# SCP_Pcanadensis_Worst_Case_Mean_3C_warmer.jpg
# SCP_Pcanadensis_Worst_Case_Mean_5C_warmer.jpg
...
setwd("01_Analysis/Spatial Predictions/Species Vulnerability/Papilio canadensis")
# setwd("Y:/kthompson/Ch 4 Bumblebees/Spatial Predictions/Species Sensitivities/Papilio canadensis/Worst Case")

dur0_mean <- raster :: raster("SCP_P.canadensis_Worst Case_Mean_External.tif")
dur3_mean <- raster :: raster("SCP_P.canadensis_Worst Case_Mean_3C warmer.tif")
dur5_mean <- raster :: raster("SCP_P.canadensis_Worst Case_Mean_5C warmer.tif")

dur0_median <- raster :: raster("SCP_P.canadensis_Worst Case_Median_External.tif")
dur3_median <- raster :: raster("SCP_P.canadensis_Worst Case_Median_3C warmer.tif")
dur5_median <- raster :: raster("SCP_P.canadensis_Worst Case_Median_5C warmer.tif")

dur0_min <- raster :: raster("SCP_P.canadensis_Worst Case_Minimum_External.tif")
dur3_min <- raster :: raster("SCP_P.canadensis_Worst Case_Minimum_3C warmer.tif")
dur5_min <- raster :: raster("SCP_P.canadensis_Worst Case_Minimum_5C warmer.tif")

dur0_max <- raster :: raster("SCP_P.canadensis_Worst Case_Maximum_External.tif")
dur3_max <- raster :: raster("SCP_P.canadensis_Worst Case_Maximum_3C warmer.tif")
dur5_max <- raster :: raster("SCP_P.canadensis_Worst Case_Maximum_5C warmer.tif")
...
path <- "01_Analysis/Figure 1/"
# path <- "L:/LabMemberFolders/KimberlyThompson/Ch 4 Bumblebees/Analysis/Graphs/Species Sensitivities/Papilio canadensis/Worst Case"
setwd(path)

for (i in nlayers(rasterlist)) {
  
  file.name <- paste(names(rasterlist)[[i]], "_Pcanadensis.jpg", sep="")
