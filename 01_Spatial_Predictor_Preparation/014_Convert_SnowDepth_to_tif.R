...
# Note: the plan was to run on HPC cluster due to memory requirements
# however, I can not get it to run without error, despite it running fine
# in segments when i test
# Therefore I will run this in batches and then combine the spatrasters
...
missing_dates <- c("2017-02-01", "2017-02-03", "2017-02-08")
...
# When i plot this, the bounding box does not look like it extends far enough
# west, so I will adjust
bbox <- c(xmin = -105, xmax = -72, ymin = 37, ymax = 55)
...
# For February remove the layer for the last day in January
# can <- can[[-1]]
